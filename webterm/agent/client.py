...
class BrokerClient:
    ...
    async def _serve(self, ws) -> None:
        ...
        await ws.send(protocol.hello_frame(
            ...
            profile=s.profile, version=s.version, pyte=s.pyte,
        ))
        self._ws = ws
        self.connected = True
        ...
        async for message in ws:
            ...
            if mtype == "input":
                ...
                if self._on_screen_request is not None:
                    ...
                    # attrs adds the styled-run map (#128); wait_for_idle drives
                    # the settle wait (#135). All absent for older brokers -> an
                    # immediate single plain-text read.
                    ...
                    self._on_screen_request(
                        ...
                        wait_for_idle=_int(data.get("wait_for_idle"), 0))
            else:
