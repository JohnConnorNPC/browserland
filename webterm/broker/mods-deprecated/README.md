...
5. **Restart the broker.** `INDEX_HTML` is assembled and cached at import, so an
   ...
   next page load, always (`docs/MODS.md` §10.3).
6. **Expect the mod to come back at its DEFAULT, not at each browser's old
   ...
   survived.
7. **Update `README.md`, `docs/MODS.md` and the wiki** — the retirement edited
   all three.
...
the next page load, always (`docs/MODS.md` §10.3) — un-shipping is not a
runtime revocation.
...
shipped mods share one concatenated script — see `docs/MODS.md` §10.2. That is
