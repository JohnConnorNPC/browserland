...
- [ ] [F077] Synced UI-state API — `GET/PUT /state` shared per-broker settings+layout blob with optimistic-concurrency `rev` (409 conflict inlines the live state; lease-gated PUT returns not_active for a non-active browser); durable self-healing `webterm_state.json` (atomic write, rev persisted across restarts); same token-or-loopback gate as /file/* (webterm/broker/app.py)
