...
- [x] [F058] In-app help guide — "?" chip opens a live-search interface guide injecting one entry per shortcut + MCP status (MCP-status row depends on F065) (index.html)
...
- [ ] [F083] Control Panel full-screen presentation — DOC/BEHAVIOR GAP (split from F060): wiki `Getting-Started.md:42` says the Control Panel "opens as a full-screen panel", but the code renders `#settings-modal` capped at `min-width:340px; max-width:480px; max-height:80vh` (a centered dialog; only the dimmed `#settings-overlay` backdrop is full-screen). Code comment (index.html:13394) notes "the floating Control Panel window was retired", so the modal is a deliberate redesign — but the user-facing wiki still promises full-screen. Functional panel (tabs+sections+routing) verified done under F060. BLOCKED: needs a human decision — either enlarge `#settings-modal` toward full-screen (index.html edit, deferred by the pre-existing uncommitted index.html change), OR correct the wiki wording to "panel/dialog". Don't auto-pick: which is the intended UX is the owner's call. TRACKED: gh #62 (relates to #59)
