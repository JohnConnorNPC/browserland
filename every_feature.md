...
- [ ] [F078] Browser-facing `GET /profiles` — names-only launchable profiles (default + list; command/cwd never leave the broker), same token-or-loopback gate as /sessions; backs the launch (+) menu, per-host remote profile groups, the default-profile picker, and "Open in folder" (distinct from /mcp/profiles) (webterm/broker/app.py)

## Window Management (frontend, index.html)

- [ ] [F023] Floating windows — free move via title-bar drag and resize via edges/corners; overlapping z-order (index.html)
...
- [ ] [F030] Tabs — Alt-drag to stack windows as tabs in one tile; tab strip switches; "Tab into left/right column" / "Tab this window" (index.html)
