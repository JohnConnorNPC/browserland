...
- [x] [F031] Untab (menu + ⊟ button) — "Untab tile (split to rows)" `untabTile`, "Untab cell (side by side)" `nestedUntabCell`, and the tab-strip ⊟ button — all persist+relayout. NOTE: the "drag a tab out" gesture was split off to F082 and RESOLVED by #61 — the gesture was never implemented and its stale doc claim was removed from `wiki/Arranging-Windows.md` (no longer advertised) (index.html)
...
- [x] [F047] File manager — dual-pane browser at active cwd/host; open files into the editor; backed by /file/* (index.html)
- [x] [F048] Task manager UI — live cross-host session list + expandable child-process tree; End process / destroy session buttons driving F013's backend contract (index.html)
...
- [x] [F052] Start button label — configurable + button label (blank → `+`); behavior unchanged (index.html)
- [x] [F053] Workspaces + pager — virtual desktops; pager dots switch; prev/next + Go-to-1–5 shortcuts; add via + dot (index.html)
...
- [x] [F058] In-app help guide — "?" chip opens a live-search interface guide; the static cards are sourced from `wiki/*.md` as the single source of truth (#60: `help_corpus.py` parses the wiki into a typed corpus served at `/help-corpus.json`; the frontend fetches it and renders typed fragments with DOM APIs only), merged with live entries injected one per shortcut + per-host profiles + MCP status (MCP-status row depends on F065) (index.html, webterm/broker/help_corpus.py)
...
- [x] [F060] Control Panel — tabbed settings hosted in a moveable floating window (appKind `control-panel`, #59: `openControlPanelWindow` borrows the `#settings-modal` node into a `.term-window`; the legacy `#settings-overlay` is no longer shown as a modal), a tab per connected broker (`renderSettingsTabs`/`showSettingsPane`, local+remotes+Browser), all settings sections present, per-host vs `.set-browser-global` routing via `settingsTarget` (local savePrefs / remote putHostState→/state). Ephemeral like Help / the task manager (`saveAppWindow` skips it) (index.html)
...
- [x] [F079] Per-host default profile + start path — per-host "Default terminal profile" (feeds the + button / Open-in-folder) and "Default start path" (roots new shells' cwd via resolveStartPath); persisted per-host and synced via /state (#17/#10) (index.html)
...
- [x] [F064] Host status chips — per-host taskbar chips ok / down / auth-needed (click to log in) / lease (click to take over); toggle host windows (index.html)
...
- [x] [F074] send_input + newline→CR mapping — `POST /mcp/input` verbatim (readwrite only, ≤256KiB, bypasses lease); tool maps \n/\r\n → CR so commands submit (PowerShell) (app.py, mcptool/server.py)
...
- [x] [F082] Untab: drag a tab out of the strip — RESOLVED by #61: removed the unimplemented "drag a tab out" help line from `wiki/Arranging-Windows.md` (and regenerated `help_corpus.json`). The gesture was never implemented (`.strip-tab` mousedown only stopPropagation, click switches activeTab; `dataset.tabKey` set but never read by a drag consumer; `startTiledDrag` has no single-tab detach mode); the owner chose the doc fix over building it. The 3 real untab paths remain (menu "Untab tile (split to rows)" `untabTile`, "Untab cell (side by side)" `nestedUntabCell`, and the tab-strip ⊟ button). Docs now match behavior. (wiki, help_corpus.json)
- [x] [F083] Control Panel presentation — RESOLVED by #59: the Control Panel is now a moveable, resizable floating window (`openControlPanelWindow`), which supersedes the old full-screen-vs-modal question entirely. The window borrows the existing `#settings-modal` node (reusing every settings renderer and the local-vs-remote routing unchanged) and hands it back on close; the retired-window guards/comments and the `openSettings()` modal redirect were removed. Docs corrected: wiki `Getting-Started.md` / `Window-Types.md` / `Home.md` now describe a floating window. (index.html, wiki)
