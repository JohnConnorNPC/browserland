...
- **Browser-global settings** follow the browser you are sitting at — theme and background, the terminal font (an opt-in mod, off by default — turn it on under **Control Panel → Mods**), the start-button label, restore-on-refresh, the workspace pager's label mode, visibility and taskbar filter, and the clock chip's time zone.
