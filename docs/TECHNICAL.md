...
**Third-party code (#143).** The page executes xterm from jsdelivr and, lazily, CodeMirror from esm.sh — in the same origin that holds `prefs._hosts[].token` for **every** configured host. Two independent controls:

* **SRI** on all four pinned jsdelivr assets (`integrity` + the `crossorigin` that makes verification possible at all). Hashes were taken from the CDN and cross-checked byte-for-byte against unpkg. A stale hash **blocks** the asset, so bump them with any version change — the regeneration one-liner is in the comment at the top of `00_head.html`.
* **`script-src`** listing only `https://cdn.jsdelivr.net` and `https://esm.sh`, plus a `'sha256-…'` for the page's single inline `<script>`. The hash is computed at startup from the assembled `INDEX_HTML` itself (`ui.inline_script_hash`), so it tracks the bundle automatically and needs no `'unsafe-inline'` — an *injected* inline script cannot execute. Nothing else is set (no `default-src`/`style-src`/`connect-src`): those would have to enumerate every inline style, the `data:` favicon, `blob:` download URLs and every federated host, each a way to break the app for no gain here.

What this does **not** do: neither control stops a compromised package on an origin that is *allowed*. SRI neutralizes that for jsdelivr; esm.sh serves deliberately-mutable range-resolved URLs (see the comment in `mods/editor/codemirror.js` — exact pins silently break syntax highlighting), so it cannot be integrity-pinned and remains trusted. Vendoring CodeMirror is the only real fix there, tracked in #143.
